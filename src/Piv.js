...
export default function Piv() {
  ...
  const doImportKey = useCallback(async () => {
    ...
    try {
      if (keyAlgo === 'RSA') {
        ...
        if (rsaDp.length === 256) dataString = dataString + '038180' + rsaDp
        else {
          let DpLen = (rsaDp.length / 2).toString(16).padStart(2, '0')
          dataString = dataString + '03' + DpLen + rsaDp
        }

        const rsaDq = base64url.toBuffer(jwk['dq']).toString('hex')
        if (rsaDq.length === 256) dataString = dataString + '048180' + rsaDq
        else {
          let DqLen = (rsaDq.length / 2).toString(16).padStart(2, '0')
          dataString = dataString + '04' + DqLen + rsaDq
        }

        const rsaQinv = base64url.toBuffer(jwk['qi']).toString('hex')
        if (rsaQinv.length === 256) dataString = dataString + '058180' + rsaQinv
        else {
          let QinvLen = (rsaQinv.length / 2).toString(16).padStart(2, '0')
          dataString = dataString + '05' + QinvLen + rsaQinv
        }

        let dataLen = (dataString.length / 2).toString(16).padStart(6, '0')
        let res = await dispatch(
          transceive(`00fe07${slotPosition}${dataLen}${dataString}`)
        )
        if (res.endsWith('9000')) {
          enqueueSnackbar(slotPosition + ':RSA key import succeed', {
            variant: 'success',
          })
        } else {
          enqueueSnackbar(slotPosition + ':RSA key import failed', {
            variant: 'error',
          })
        }
      } else {
        ...
        if (algoFlag.startsWith('P-256')) {
          if (priKey.length !== 64)
            throw new Error('The length of the ec-key is wrong!')
          res = await dispatch(
            transceive(`00fe11${slotPosition}220620${priKey}`)
          )
        } else if (algoFlag.startsWith('P-384')) {
          if (priKey.length !== 96)
            throw new Error('The length of the ec-key is wrong!')
          res = await dispatch(
            transceive(`00fe14${slotPosition}320630${priKey}`)
          )
        }
        if (res.endsWith('9000')) {
          enqueueSnackbar(slotPosition + ':ECC key import succeed', {
            variant: 'success',
          })
        } else {
          enqueueSnackbar(slotPosition + ':ECC key import failed', {
            variant: 'error',
          })
        }
      }
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      if (device !== null && MKValid && !stopRefresh) {
        let res = await dispatch(
          transceive(`00cb3fff0000055c03${slotMap[slotPosition]}`)
        )
  ...
  const doDeleteCrt = useCallback(async () => {
    try {
      let res = await dispatch(
        transceive(`00db3fff055c03${slotMap[slotPosition]}`)
      )
      if (res.endsWith('9000')) {
  ...
  const doCreatCrt = useCallback(async () => {
    ...
    try {
      ...
      if (algo === 'RSA-2048') {
        keyRes = await dispatch(
          transceive(`004700${slotPosition}000005AC03800107`)
        )
        ...
        let modulus = base64url(
          Buffer.from(hexStringToByte(keyRes.substr(18, 512)))
        )
        let exponent = base64url(
          Buffer.from(hexStringToByte(keyRes.substr(534, 8)))
        )
        publicJwk = { kty: 'RSA', n: modulus, e: exponent }
      } else {
        keyRes = await dispatch(
          transceive(
            `004700${slotPosition}05AC038001${
              algo === 'ECC-P-256' ? '11' : '14'
            }`
          )
        )
        if (keyRes.endsWith('9000')) {
      ...
      x509
        ...
        .then(async (crt) => {
          ...
          const outerLength = (derCrt.length / 2 + 9)
            .toString(16)
            .padStart(6, '0')
          const innerLength = (derCrt.length / 2).toString(16).padStart(4, '0')
          let res = await dispatch(
            transceive(
              `00db3fff${outerLength}5c03${slotMap[slotPosition]}5382${innerLength}${derCrt}`
            )
          )
          if (res.endsWith('9000')) {
  ...
  const doChangePin = useCallback(async () => {
    ...
    try {
      ...
      let res = await dispatch(
        transceive(`0024008010${oldHexString}${newHexString}`, true)
      )
      if (res.endsWith('9000')) {
  ...
  return (
    <div className={classes.root}>
            ...
            <Card className={classes.card}>
              ...
              <TabPanel value={tabValue} index={0}>
                <Typography variant="h5">
                  {ownCrt
                    ? 'You have a certificate in this slot.'
      ...
      <Dialog
        open={creatCrtDialogOpen}
        onClose={() => setCreatCrtDialogOpen(false)}
      >
        ...
        <DialogActions>
          <Button variant="contained" color="primary" onClick={doCreatCrt}>
            Creat
          </Button>
        </DialogActions>
      </Dialog>
